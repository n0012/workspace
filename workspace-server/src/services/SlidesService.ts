...
export class SlidesService {
  ...
  /** Find a page element by id, descending into element groups. */
  ...
  /**
   * Replace a shape's text in place and apply explicit styling. The safe analog
   * of a raw insertText: it clears the shape, inserts the new text, and sets each
   * style attribute with an explicit fields mask so text never inherits stray
   * styling. Only the style fields provided are changed.
   */
  public setText = async ({
    ...
    try {
      ...
      if (style) {
        ...
        // Ranged emphasis within the new text. Skip empty phrases — an empty
        // search string yields zero-length matches the API rejects.
        for (const phrase of style.bold_phrases ?? []) {
          if (!phrase) continue;
          let from = 0;
          let idx: number;
